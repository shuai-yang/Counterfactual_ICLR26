    ...
    if (dataset_name == 'CRASS' and mode == 'zero_shot'):
        ...
        **Counterfactual Graph Requirements:**
        ...
        ## Validation Checklist
         Factual edges ≥6 | Counterfactual edges ≥5  
         All Z relationships preserved in counterfactual  
         Prime notation used for modified variables  
         No missing core relationships (Z→X, X→M→Y, etc.)

        ## Example Edge Structures
        **Factual:**
        ["low income", "received tutoring"],
        ["received tutoring", "7 study hours"],
        ["7 study hours", "score=85"]

        **Counterfactual:** 
        ["low income", "5 study hours"],
        ["no tutoring", "5 study hours"], 
        ["5 study hours", "score=78"]
        
        **STRICT REQUIREMENTS:**
        1. JSON keys MUST use EXACTLY these names with NO whitespace:
        - "factual_roles" 
        - "counterfactual_roles" 
        - "causal_graph"
        2. Remove all line breaks inside JSON strings
        3. Use UNIX line endings (\n), NOT Windows (\r\n)
        4. Ensure all closing brackets are properly placed
        '''
